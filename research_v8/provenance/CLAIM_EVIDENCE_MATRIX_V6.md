...
| C6 | The resulting stegosystem is operationally secure against contemporary steganalysis at controlled payload. | Actual coding plus matched payload; SRM/SRNet-or-stronger; repeated splits/seeds; uncertainty; traceable raw evidence. | V8 coding/raw lineage is now closed by a clean 20,000-record rerun from the tracked vendored STC source: 5000/5000, 5000/5000, 4998/5000, and 4999/5000 successful encodings; every successful encoding has zero bit errors. The six historical raw blocks (0–1500; 6000 records) reproduce exactly on every scientific field except elapsed time, and the same three failures recur in the full rerun. Strong detector-facing steganalysis remains separate. | OPEN — CODING + RAW PROVENANCE PASS; STRONG DETECTOR PENDING |
...
- `operational/V8_STC_RUNTIME_PROVENANCE_AND_RERUN_CLOSURE.md`
- `results/V8_STC_RERUN/V8_STC_GITHUB_MANIFEST.sha256`
- `results/V8_STC_RERUN/V8_STC_RERUN_SUMMARY.json`
